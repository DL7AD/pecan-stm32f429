...
void replace_placeholders(char* fskmsg, uint16_t size, trackPoint_t *trackPoint) {
	...
	chsnprintf(buf, sizeof(buf), "%d.%07d", trackPoint->gps_lat/10000000, trackPoint->gps_lat%10000000+10000000); // TODO: this calculation does not work in other hemispheres
	str_replace(fskmsg, size, "<LAT>", buf);
	chsnprintf(buf, sizeof(buf), "%d.%07d", trackPoint->gps_lon/10000000, trackPoint->gps_lon%10000000+10000000); // TODO: this calculation does not work in other hemispheres
