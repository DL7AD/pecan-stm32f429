...
const char* const fileIDs[] = {
	"",				// 0 (no error, used for time padding, if no error occured since 2^16 seconds)
...
error_t error_log[ERRORLOG_SIZE];	// Error log ring memory
uint8_t error_id;					// Incremental ID for ring memory
...
uint32_t pkt[ERRORLOG_SIZE+1];
uint8_t pkt_base91[BASE91LEN(4*ERRORLOG_SIZE+4)];
	...
	while(true)
	{
		...
		if(!p_sleep(&config->sleep_config))
		{
			...
			switch(config->protocol) {
				...
				default:
					TRACE_ERROR("IMG  > Unsupported protocol selected for module LOG");
			}
		}
