...
void initModules(void)
{
	...
	chsnprintf(config[0].aprs_config.path, 7, "WIDE1-1");	// APRS Path
	...
	chsnprintf(config[1].aprs_config.path, 7, "WIDE1-1");	// APRS Path
	...
	config[2].power = 13;									// Power 10 dBm
	...
	config[2].sleep.types[0] = SLEEP_DISABLED;				// Sleep when battery below threshold
	...
	config[4].power = 20;									// Power 10 dBm
	...
	config[4].init_delay = 1000;							// Module startup delay in sec
	...
	config[4].ssdv_config.res = RES_VGA;					// Resolution VGA
	...
	config[5].init_delay = 1000;							// Module startup delay in sec
	...
	config[5].trigger.timeout = 600;						// Transmit continously
	...
	config[6].power = 13;									// Power 10 dBm
	...
	config[6].sleep.types[0] = SLEEP_DISABLED;				// Sleep when battery below threshold
	config[6].trigger.type = TRIG_TIMEOUT;					// Trigger transmission on timeout (Periodic cycling)
	config[6].trigger.timeout = 600;						// Transmit continously
	...
	config[6].ssdv_config.res = RES_VGA;					// Resolution QVGA
